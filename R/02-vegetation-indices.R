  ...
  if (!is.null(region_boundary)) {
    ...
    tryCatch({
      ...
      if (is.character(region_boundary)) {
        ...
        # Treat as c(xmin, ymin, xmax, ymax) bounding box
        if (verbose) message("Using numeric bounding box as boundary")
        boundary <- sf::st_as_sf(sf::st_as_sfc(sf::st_bbox(
          c(xmin = region_boundary[1], ymin = region_boundary[2],
            xmax = region_boundary[3], ymax = region_boundary[4]),
          crs = sf::st_crs(4326)
        )))
      } else {
        stop("region_boundary must be either an sf object, file path, region name, or numeric bbox c(xmin,ymin,xmax,ymax)", call. = FALSE)
      }
